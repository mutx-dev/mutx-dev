...
# Ensure mutx SDK is importable (matches conftest.py SDK path setup)
...
def _steps_payload() -> list[dict[str, Any]]:
    return [
        {
            "type": "tool_call",
...
def test_report_run_posts_to_runs_endpoint() -> None:
    ...
    with httpx.Client(
        ...
        run = obs.report_run(input_run)

    assert captured["path"] == "/v1/observability/runs"
    assert captured["json"]["agent_id"] == input_run["agent_id"]
...
def test_submit_eval_posts_eval_data() -> None:
    ...
    with httpx.Client(
        ...
        obs = Observability(client)
        obs.submit_eval(run_id, eval_data)
...
async def test_asubmit_eval_posts_eval_data() -> None:
    ...
    eval_data = {
        ...
        "expected_outcome": "function returns correct output",
        "actual_outcome": "function returns correct output",
        "metrics": {"cost_usd": 0.02, "duration_s": 8.0, "tool_calls": 2},
        "benchmark_id": "math-500",
    }
    ...
    async with httpx.AsyncClient(
        ...
        obs = Observability(client)
        await obs.asubmit_eval(run_id, eval_data)
...
def test_require_async_client_emits_deprecation_warning() -> None:
    """Verify _require_async_client emits DeprecationWarning when client is sync."""
    import warnings

    sync_client = httpx.Client(base_url="https://api.test")
    obs = Observability(sync_client)

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        obs._require_async_client()
...
def test_build_run_from_steps_generates_unique_ids() -> None:
