...
def test_release_contract_is_immutable_and_digest_pinned() -> None:
    ...
    assert check_faramesh_v020_compat.RELEASE_REF == ("ae3ebc9066d65e4e930164881c2f2ce2be554c7f")
...
def test_version_detection_uses_published_cli_flag(
    mock_run: MagicMock, _mock_bin: MagicMock
) -> None:
...
def test_gate_decide_sends_v020_govern_shape(
    ...
    mock_send.return_value = [{"call_id": "call-1", "effect": "PERMIT", "latency_ms": 2}]
...
def test_approval_and_kill_operations_match_v020_socket_contract(mock_send: MagicMock) -> None:
    ...
    assert (
        faramesh_runtime.kill_agent("/tmp/faramesh.sock", "agent-1", admin_token="secret") is True
    )
    assert mock_send.call_args.args[1] == {
        "type": "kill",
        "agent_id": "agent-1",
...
def test_gateway_normalizes_v020_pending_token() -> None:
    ...
    assert pending == [{"token": "defer-1", "defer_token": "defer-1", "agent_id": "agent-1"}]
...
@pytest.mark.parametrize("name", ["FAREMESH_POLICY_ADMIN_TOKEN", "FAREMESH_STANDING_ADMIN_TOKEN"])
def test_kill_agent_reads_official_token_prefix(monkeypatch, name):
    for variable in (
        "FAREMESH_POLICY_ADMIN_TOKEN",
        "FAREMESH_STANDING_ADMIN_TOKEN",
        "FARAMESH_POLICY_ADMIN_TOKEN",
        "FARAMESH_STANDING_ADMIN_TOKEN",
    ):
        monkeypatch.delenv(variable, raising=False)
    monkeypatch.setenv(name, "configured-test-token")
    with patch("cli.faramesh_runtime._send_socket_request", return_value=[{"ok": True}]) as send:
        assert faramesh_runtime.kill_agent("/tmp/faramesh.sock", "agent-1")
    assert send.call_args.args[1]["admin_token"] == "configured-test-token"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [{"status": "unknown"}],
        [{"error": "unavailable"}],
        [{"status": "approved", "authoritative": False}],
    ],
)
def test_v020_poll_does_not_permit_indeterminate_responses(responses):
    with patch("cli.faramesh_runtime._send_socket_request", return_value=responses):
        result = faramesh_runtime.wait_for_decision("defer-1", timeout=1, agent_id="agent-1")
    assert not result.executed
    assert not result.is_authoritative_permit
