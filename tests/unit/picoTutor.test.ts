...
describe('Pico tutor', () => {
  it('grounds install questions in the setup lessons', () => {
    const answer = answerTutorQuestion('Why is hermes not on my path after install?')
  ...
  it('escalates risky questions instead of bluffing', () => {
    const answer = answerTutorQuestion('I lost a production key and there may be a security breach')
  ...
  it('points workflow questions at the scheduling lesson', () => {
    const answer = answerTutorQuestion('How do I schedule a cron workflow for my agent?')
