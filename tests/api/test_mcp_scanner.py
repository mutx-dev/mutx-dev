...
def test_hidden_instructions_in_metadata_keys_are_scanned(location: str):
    ...
    if location == "schema":
        definition["tools"][0]["inputSchema"]["properties"][poisoned_key] = {"type": "string"}
    else:
...
def scanner_viewer(test_user):
    test_user.roles = ["VIEWER"]


@pytest.mark.parametrize("args", [["-rf", "/"], ["-fr", "/"], ["-f", "-r", "/"]])
def test_destructive_rm_flag_order(args):
    definition = safe_server_definition()
    definition.update(command="rm", args=args, transport="stdio")
    result = scan_mcp_definition(definition)
    assert "destructive_startup_command" in finding_codes(result)
    assert not result.registration_allowed


@pytest.mark.parametrize(
    "args", [["-lc", "echo hello"], ["-ec", "echo hello"], ["-o", "pipefail", "-c", "echo hello"]]
)
def test_shell_eval_combined_flags(args):
    definition = safe_server_definition()
    definition.update(command="bash", args=args, transport="stdio")
    result = scan_mcp_definition(definition)
    assert not result.registration_allowed


def test_unrestricted_nullable_command_is_denied():
    definition = safe_server_definition()
    definition["tools"][0].update(
        name="shell.exec",
        inputSchema={"type": "object", "properties": {"command": {"type": ["string", "null"]}}},
    )
    assert not scan_mcp_definition(definition).registration_allowed
