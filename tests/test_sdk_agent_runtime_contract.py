...
async def test_mutx_agent_client_register_hits_contract_route():
    ...
    async def mock_post(*args, **kwargs):
        captured["path"] = args[0] if len(args) > 0 else ""
...
def test_mutx_agent_sync_client_register():
    ...
    def mock_post(*args, **kwargs):
        ...
        response = httpx.Response(
            201,
            request=httpx.Request("POST", "/v1/agents/register"),
            json=_agent_info_response(agent_id=agent_id, api_key=api_key),
        )
        return response
...
def test_mutx_agent_sync_client_heartbeat():
    ...
    def mock_post(*args, **kwargs):
        ...
        captured["json"] = kwargs.get("json", {})
        response = httpx.Response(
            200, request=httpx.Request("POST", "/v1/agents/heartbeat"), json={"status": "ok"}
        )
        return response
