...
def test_launch_profile_parser_accepts_optional_faramesh_policy() -> None:
    payload = _profile_payload()
    profile = LaunchProfile(payload)
...
def test_list_agents_hits_correct_route() -> None:
    ...
    agents_data = [_agent_payload(), _agent_payload(status="stopped")]
...
async def test_alist_agents_hits_correct_route() -> None:
    ...
    assert captured["path"] == "/runtime/governance/supervised/"
    assert len(agents) == 1
    assert agents[0].agent_id == agents_data[0]["agent_id"]
...
def test_list_profiles_hits_correct_route() -> None:
    captured: dict[str, Any] = {}
    profiles_data = [_profile_payload(), _profile_payload(name="debug")]

    def handler(request: httpx.Request) -> httpx.Response:
        captured["path"] = request.url.path
        return httpx.Response(200, json=profiles_data)
...
async def test_alist_profiles_hits_correct_route() -> None:
    captured: dict[str, Any] = {}
    profiles_data = [_profile_payload(name="async-profile")]
