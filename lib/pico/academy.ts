...
export const PICO_LESSONS: PicoLesson[] = [
  {
    ...
    summary: 'Get Hermes installed, open the CLI once, and stop the second the command works from a fresh shell.',
    level: 0,
    track: 'first-agent',
    estimatedMinutes: 8,
    difficulty: 'setup',
    objective: 'Install Hermes and finish one working local provider setup without branching into extra tooling.',
    prerequisites: [],
    outcome: 'A working local Hermes install with one usable provider path.',
    expectedResult: 'From a fresh shell, `hermes` opens cleanly and you are ready to run the first prompt.',
    validation: 'From a fresh shell run `command -v hermes && hermes` and verify the CLI opens without install or provider errors.',
    xp: 60,
    ...
    steps: [
      ...
      {
        title: 'Finish one provider setup completely',
        body: 'Choose exactly one provider path and finish it now. Do not leave the lesson with two half-configured options and zero working runtime.',
      },
    ],
    troubleshooting: [
  ...
  },
  {
    ...
    summary: 'Send one tiny real prompt through Hermes, get one visible answer back, and keep the transcript instead of a vague memory.',
    level: 0,
    track: 'first-agent',
    estimatedMinutes: 6,
    difficulty: 'setup',
    objective: 'Prove the runtime can answer one bounded prompt with the model you configured.',
    prerequisites: ['install-hermes-locally'],
    outcome: 'One verified local transcript you can reuse later.',
    expectedResult: 'Hermes answers a tiny real prompt and you keep the prompt plus output in a file or note.',
    validation: 'Run one bounded prompt, confirm the answer is visible immediately, and save both the prompt and the answer in a note or file you can reopen later.',
    xp: 60,
    ...
    steps: [
      ...
      {
        title: 'Save a reusable transcript',
        body: 'Store the prompt and answer in a note or file, not just a screenshot. A transcript is something you can compare against later; a screenshot is mostly decoration.',
      },
    ],
    troubleshooting: [
  ...
  },
  {
    ...
    summary: 'Write a real service definition so logging out does not kill the runtime.',
    level: 1,
    track: 'deployed-agent',
    estimatedMinutes: 25,
    difficulty: 'operator',
    ...
    outcome: 'A service or tmux definition that keeps Hermes alive after logout.',
    expectedResult: 'A named service definition exists and the runtime survives a disconnect or restart.',
    validation: 'Create the service definition, start it, disconnect, reconnect, and verify the process is still up.',
    xp: 55,
    steps: [
      {
        title: 'Write the service definition',
        body: 'Create the actual `systemd` unit file or named tmux command now. If the lesson ends without a file or command you can rerun, nothing was shipped.',
      },
      {
        title: 'Point it at the verified runtime',
        body: 'Use the exact Hermes binary, working directory, and environment that already worked manually. Do not invent a new path here.',
      },
      {
  ...
  },
  {
    ...
    summary: 'Give the agent one real ingress path so you can reach it without SSH.',
    level: 1,
    track: 'deployed-agent',
    estimatedMinutes: 30,
    difficulty: 'operator',
    objective: 'Attach one real ingress path like Telegram, Discord, Slack, or a simple webhook lane.',
    prerequisites: ['keep-your-agent-alive'],
    outcome: 'One tested ingress path into the runtime.',
    expectedResult: 'A real input reaches the runtime from the selected channel or interface and produces one visible reply.',
    validation: 'Send one real test input through the chosen ingress and confirm the runtime receives it and answers once.',
    xp: 55,
    steps: [
      {
        title: 'Choose one channel',
        body: 'Pick the channel or interface you will actually use first. One real ingress beats pretending to support everything.',
      },
      {
      ...
      {
        title: 'Run the ingress smoke test',
        body: 'Send one short real input and confirm the runtime answers exactly once in the expected place.',
      },
    ],
    troubleshooting: [
      ...
      'If chat is the wrong surface, use a webhook or HTTP ingress and document that exact choice instead of skipping the lesson.',
    ],
  ...
  },
  {
    ...
    summary: 'Open the control surface, inspect one real run, and capture the exact signal you will watch from now on.',
    level: 4,
    ...
    outcome: 'A visible run, alert, or honest empty-state baseline from the control layer.',
    expectedResult: 'You can point to one concrete runtime signal: a run id, an alert, or the current empty-state truth.',
    validation: 'Open the autopilot surface and record one concrete signal you can reference later: a run id, an alert type, or the current empty state.',
    xp: 70,
    milestoneEvents: ['first_monitoring_event_seen'],
    steps: [
      {
        title: 'Open the control view and pick one signal',
        body: 'Look at runs first. Your job is not to browse. Your job is to leave with one concrete runtime signal you can name later.',
      },
      {
        title: 'Inspect one run or one empty state',
        body: 'Read the status, timing, and output. If there is no run yet, record that empty state honestly instead of pretending visibility exists.',
      },
      {
        title: 'Capture the first operator checkpoint',
        body: 'Write down the run id, alert type, or empty-state fact you will use as your first operator checkpoint.',
      },
    ],
    troubleshooting: [
