...
function NavTree({ item, pathname }: { item: NavItem; pathname: string }) {
  const isActive =
    pathname === `/docs/${item.slug}` || pathname === `/docs/${item.slug}/`;
  ...
  return (
    <>
      <Link
        ...
        style={{ paddingLeft: `${12 * item.depth + 8}px` }}
      >
        {item.title}
      </Link>
      {hasChildren && (
        <div>
          {item.children.map((child) => (
            <NavTree key={child.slug} item={child} pathname={pathname} />
          ))}
        </div>
      )}
    </>
...
export default function DocsLayout({ nav, children, title }: DocsLayoutProps) {
      ...
      <div className="flex relative">
        ...
        >
          <nav className="py-4 pr-4">
            {nav.map((item) => (
              <NavTree key={item.slug} item={item} pathname={pathname} />
            ))}
          </nav>
        </aside>
