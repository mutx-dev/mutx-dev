  ...
  return withErrorHandling(async (req: Request) => {
    ...
    if (agentId) {
      ...
      if (agentResponse.status === 403) {
        return forbidden('Not authorized to view deployments for this agent')
      }
    ...
    if (response.status === 403) {
      return forbidden('Not authorized to access deployments')
    }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  return withErrorHandling(async (req: Request) => {
    ...
    // Validate ownership if agent_id is provided
    if (body.agent_id) {
      const user = await getCurrentUser(request)
      if (!user) {
        return unauthorized()
      ...
      if (agentResponse.status === 403) {
        return forbidden('Not authorized to deploy this agent')
      }
