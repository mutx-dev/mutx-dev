...
| ------------ | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| Web          | landing site, dashboard routes, observability dashboard, and control demo all exist in Next.js | dashboard still does not cover every backend capability; some flows remain better in CLI or direct API                                           | fill dashboard gaps, tighten auth/session UX, keep demo vs live boundaries honest    |
| API          | real `/v1/*` contract with auth, agents, deployments, runs, monitoring, webhooks, budgets, leads, governance, observability, and more | RAG search and scheduler return 503 with feature flags until runtime is configured; shell pages (channels, skills, orchestration) are placeholder-backed     | auth hardening, route ownership checks, typed response polish, lifecycle tests       |
...
| Tests and CI | API, CLI, frontend, observability, and docs tests exist (250+ tests) | docs drift tests were too narrow; hosted-vs-local assumptions still need careful coverage                                                        | route/openapi drift checks, link checks, local-first validation                      |
| Docs         | docs are now structured for GitBook and GitHub together; v1.2 release notes added | drift risk remains high whenever routes, app paths, or CLI groups move                                                                           | doc drift guardrails, GitBook sync rules, API reference upkeep                       |
