...
_DESTRUCTIVE_COMMAND_PATTERN = re.compile(
    r"(?:\brm\s+(?=(?:-[a-z]+\s+)*-[a-z]*r)(?=(?:-[a-z]+\s+)*-[a-z]*f)(?:-[a-z]+\s+)+[/~*]|\b(?:mkfs|dd)\b|\bchmod\s+-R\s+777\b)",
    re.IGNORECASE,
)
...
_SHELL_EVAL_PATTERN = re.compile(
    r"\b(?:ba|z|fi|da)?sh\b(?:\s+(?:-[a-z]+|pipefail)){0,8}\s+-[a-z]*c[a-z]*\b", re.IGNORECASE
)
...
def _is_unrestricted_string(definition: Mapping[str, object]) -> bool:
    ...
    if declared_type not in (None, "string") and not (
