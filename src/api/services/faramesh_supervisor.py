...
Uses the `.faramesh/bin/agent` launcher generated by `faramesh apply` to
provide governance supervision for agent processes.
...
class SupervisionConfig:
    ...
    agent_launcher: Optional[str] = None
...
class FarameshSupervisor:
    """
    Supervises agent processes with Faramesh's apply-generated launcher.
    ...
    def _find_agent_launcher(self) -> Optional[str]:
        candidates = []
        if self.config.agent_launcher:
            candidates.append(Path(self.config.agent_launcher).expanduser())
        candidates.append(Path.cwd() / ".faramesh" / "bin" / "agent")
    ...
    def _resolve_trusted_policy_path(self, faramesh_policy: Optional[str]) -> Optional[str]:
        ...
        raise SupervisionValidationError(
            "Per-launch Faramesh policy overrides are unsupported by the v0.2.0 launcher; "
            "apply the policy through governance.fms before starting an agent"
        )
    ...
    async def start_prepared_agent(self, prepared: PreparedLaunch) -> bool:
        ...
        agent_launcher = self._find_agent_launcher()
        if not agent_launcher:
            logger.error(
                "Faramesh agent launcher not found; run `faramesh apply` or configure "
        ...
        if policy:
            logger.error(
                "Per-launch Faramesh policies are not supported; apply %s through governance.fms",
                policy,
            )
            return False

        supervised_command = [agent_launcher, *command]

        full_env = os.environ.copy()
        if env:
            full_env.update(env)
        full_env["FAREMESH_SOCKET"] = self.config.socket_path
    ...
    if _supervisor is None:
        ...
        _supervisor = FarameshSupervisor(
            SupervisionConfig(
                ...
                agent_launcher=settings.governance_supervised_agent_launcher,
            )
        )
    return _supervisor
