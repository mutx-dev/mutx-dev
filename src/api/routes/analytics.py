...
from src.api.models.schemas import (
    AnalyticsSummaryResponse, AgentMetricsSummary, AnalyticsTimeSeriesResponse,
    AnalyticsTimeSeries, CostSummaryResponse, BudgetResponse,
)
...
async def get_analytics_summary(
    ...
    if agent_ids:
        ...
        active_deployments = sum(1 for d in deployments if d.status in ["running", "ready", "deploying"])
    else:
    ...
    api_call_query = select(func.count()).select_from(UsageEvent).where(
        and_(UsageEvent.user_id == current_user.id, UsageEvent.event_type == "api_call",
             UsageEvent.created_at >= period_start_dt, UsageEvent.created_at <= period_end_dt)
    )
    ...
    if agent_ids:
        latency_query = select(func.avg(Metrics.latency)).where(
            and_(Metrics.agent_id.in_(agent_ids), Metrics.timestamp >= period_start_dt, Metrics.timestamp <= period_end_dt)
        )
        result = await db.execute(latency_query)
        avg_latency_ms = result.scalar_one() or 0.0
    
    return AnalyticsSummaryResponse(
        total_agents=total_agents, active_agents=active_agents, total_deployments=total_deployments,
        active_deployments=active_deployments, total_runs=total_runs, successful_runs=successful_runs,
        failed_runs=failed_runs, total_api_calls=total_api_calls, avg_latency_ms=round(avg_latency_ms, 2),
...
async def get_agent_metrics_summary(
    ...
    runs_result = await db.execute(select(AgentRun).where(
        and_(AgentRun.agent_id == agent_id, AgentRun.started_at >= period_start_dt, AgentRun.started_at <= period_end_dt)
    ))
    runs = runs_result.scalars().all()
    total_runs = len(runs)
    successful_runs = sum(1 for r in runs if r.status == "completed")
    ...
    metrics_result = await db.execute(select(AgentMetric).where(
        and_(AgentMetric.agent_id == agent_id, AgentMetric.timestamp >= period_start_dt, AgentMetric.timestamp <= period_end_dt)
    ))
    metrics = metrics_result.scalars().all()
    avg_cpu = sum(m.cpu_usage for m in metrics if m.cpu_usage) / len(metrics) if metrics else None
    avg_memory = sum(m.memory_usage for m in metrics if m.memory_usage) / len(metrics) if metrics else None
    
    sys_result = await db.execute(select(Metrics).where(
        and_(Metrics.agent_id == agent_id, Metrics.timestamp >= period_start_dt, Metrics.timestamp <= period_end_dt)
    ))
    sys_metrics = sys_result.scalars().all()
    total_requests = sum(m.requests for m in sys_metrics)
    avg_latency = sum(m.latency for m in sys_metrics if m.latency) / len(sys_metrics) if sys_metrics else None
    
    return AgentMetricsSummary(
        agent_id=agent_id, agent_name=agent.name, total_runs=total_runs, successful_runs=successful_runs,
        failed_runs=failed_runs, avg_cpu=round(avg_cpu, 2) if avg_cpu else None,
        avg_memory=round(avg_memory, 2) if avg_memory else None, total_requests=total_requests,
...
async def get_analytics_timeseries(
    metric: str = Query(...), interval: str = Query("hour"),
    period_start: Optional[str] = Query(None), period_end: Optional[str] = Query(None),
    ...
    if metric == "runs":
        ...
        q = select(func.date_trunc(interval, UsageEvent.created_at).label("ts"), func.count().label("count")
        ).where(and_(UsageEvent.user_id == current_user.id, UsageEvent.event_type == "api_call",
                     UsageEvent.created_at >= period_start_dt, UsageEvent.created_at <= period_end_dt))
        q = q.group_by("ts").order_by("ts")
        for row in await db.execute(q):
            data.append(AnalyticsTimeSeries(timestamp=row.ts.replace(tzinfo=timezone.utc) if row.ts else now, value=row.count))
    ...
    return AnalyticsTimeSeriesResponse(metric=metric, interval=interval, data=data, period_start=period_start_dt, period_end=period_end_dt)
...
async def get_cost_summary(
    ...
    result = await db.execute(select(UsageEvent).where(
        and_(UsageEvent.user_id == current_user.id, UsageEvent.created_at >= period_start_dt, UsageEvent.created_at <= period_end_dt)
    ))
    ...
    return CostSummaryResponse(total_credits_used=total_credits, credits_remaining=max(0, credits_total - total_credits),
        credits_total=credits_total, usage_by_event_type=usage_by_event_type, usage_by_agent=usage_by_agent,
        period_start=period_start_dt, period_end=period_end_dt)

@router.get("/budget", response_model=BudgetResponse)
async def get_budget(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    from src.api.models.plan_tiers import PlanTier
    result = await db.execute(select(func.sum(UsageEvent.credits_used)).where(UsageEvent.user_id == current_user.id))
    credits_used = result.scalar_one() or 0.0
    plan_credits = {PlanTier.FREE: 100, PlanTier.STARTER: 1000, PlanTier.PRO: 10000, PlanTier.ENTERPRISE: 100000}
    credits_total = plan_credits.get(current_user.plan, 100)
    credits_remaining = max(0, credits_total - credits_used)
    now = datetime.now(timezone.utc)
    reset_date = datetime(now.year + 1 if now.month == 12 else now.year, 1 if now.month == 12 else now.month + 1, 1, tzinfo=timezone.utc)
