...
async def evaluate_policies_and_request_approval(
    ...
    """Evaluate tenant policies and persist an idempotent, linked approval.
    ...
    response.approval_request = approval_request_from_record(
        created.record, user_id=user.id, roles=await get_persisted_roles(db, user)
    )
