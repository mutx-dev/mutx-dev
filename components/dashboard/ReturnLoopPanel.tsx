  ...
  switch (tone) {
    case "danger":
      return {
        ...
        button:
          "border-rose-400/24 bg-rose-400/10 text-rose-100 hover:bg-rose-400/14",
        icon: "text-rose-200",
      };
    case "warning":
      return {
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <span
            className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] ${tone.badge}`}
          >
            {card.eyebrow}
          </span>
        ...
        <div
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] border border-white/10 bg-black/20 ${tone.icon}`}
        >
...
export function ReturnLoopPanel({
  ...
  const [returnState, setReturnState] = useState<DashboardReturnState | null>(
    null,
  );
  ...
  const cards = useMemo(() => {
    ...
    const checkedToday = hasCheckedToday(
      returnState?.lastCheckedAt ?? null,
      now,
    );
    const lastCheckedTimestamp =
      getTimestamp(returnState?.lastCheckedAt) ?? getDayStartTimestamp(now);
    ...
    const failedRunsSinceLastCheck = runsSinceLastCheck.filter(
    ...
    const failureCount =
      failedRunsSinceLastCheck.length + openAlertsSinceLastCheck.length;
    const latestFailure =
            ...
            : {
                ...
                href:
                  liveAgentCount > 0
                    ...
                    : "/dashboard/agents",
                ctaLabel:
                  liveAgentCount > 0 ? "Inspect sessions" : "Open agents",
                tone: "info",
                icon: Activity,
              };
    ...
    const resumeCard: ActionCard = resumeVisit
      ? {
          ...
          detail: `Last thing you were doing ${formatRelativeTime(resumeVisit.visitedAt)}.${resumeVisit.context ? ` Context: ${resumeVisit.context}.` : ` ${normalizeSentence(resumeVisit.description)}.`}`,
          href: resumeVisit.href,
          ctaLabel: `Resume ${resumeVisit.title}`,
          tone: "info",
          icon: Clock3,
      ...
      : {
          ...
          detail:
            "As soon as you move past overview, MUTX pins the exact surface so the next visit starts in motion instead of from scratch.",
    ...
    const urgencyCard: ActionCard = !checkedToday
      ...
      : failureCount > 0
        ? {
            ...
            detail:
              latestFailure ?? "The run surface is telling you to look closer.",
            href: "/dashboard/monitoring",
