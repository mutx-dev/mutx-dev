...
function pickBudget(payload: unknown): Budget | null {
  ...
  if (isRecord(payload)) {
    const budgetItems = normalizeCollection<Budget>(payload, [
      "items",
      "budgets",
      "data",
    ]);
...
export function DashboardOverviewPageClient() {
  ...
  const [openclawRuntime, setOpenclawRuntime] =
    useState<OpenClawRuntimeSnapshot | null>(null);
  const [openclawOnboarding, setOpenclawOnboarding] =
    useState<OpenClawOnboardingState | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const payload = await readJson<DashboardOverviewPayload>(
        ...
        const coreHasData = coreResults.some(
        ...
        if (!coreHasData && coreHardFailure) {
          throw new Error(
            coreHardFailure.error || "Failed to load dashboard overview",
          );
        }
        ...
        setAgents(
          agentsResult.status === "ok"
            ? normalizeCollection<Agent>(agentsResult.data, [
                "agents",
                "items",
                "data",
              ])
        ...
        setDeployments(
          deploymentsResult.status === "ok"
            ? normalizeCollection<Deployment>(deploymentsResult.data, [
                "deployments",
                "items",
                "data",
              ])
        ...
        setRuns(
          runsResult.status === "ok" ? (runsResult.data?.items ?? []) : [],
        );
        setAlerts(
          alertsResult.status === "ok" ? (alertsResult.data?.items ?? []) : [],
        );
        setWebhooks(
          webhooksResult.status === "ok"
            ? normalizeCollection<WebhookSummary>(webhooksResult.data, [
                "webhooks",
                "items",
                "data",
              ])
        ...
        setBudget(
          budgetResult.status === "ok" ? pickBudget(budgetResult.data) : null,
        );
        setHealth(
          healthResult.status === "ok" ? (healthResult.data ?? null) : null,
        );
        setOpenclawRuntime(
          runtimeResult.status === "ok" ? runtimeResult.data : null,
        );
  ...
  const liveAgents = agents.filter((agent) =>
    ["running", "healthy"].includes(agent.status),
  );
  ...
  const openclawStatus =
    openclawRuntime?.status ?? openclawOnboarding?.status ?? "unknown";
  ...
  const briefingBarEntries = [
    ...
    },
    {
      ...
      status: (liveAgents.length > 0 ? "healthy" : "unknown") as
        | "healthy"
        | "degraded"
        | "critical"
        | "unknown",
    ...
    },
    {
      ...
      status: (unresolvedAlerts.length > 0 ? "degraded" : "healthy") as
        | "healthy"
        | "degraded"
        | "critical"
        | "unknown",
  ...
  return (
    <div className="space-y-4">
      ...
      {authRequired ? (
        ...
        <LiveErrorState title="Overview unavailable" message={error} />
      ) : (
        <>
          <LiveKpiGrid>
            <LiveStatCard
              ...
              status={asDashboardStatus(
                liveAgents.length > 0 ? "running" : "idle",
              )}
            />
            <LiveStatCard
              label="Deployments"
              ...
              status={asDashboardStatus(
                activeDeployments.length > 0 ? "healthy" : "idle",
              )}
            />
            <LiveStatCard
              label="Runs"
              ...
              status={asDashboardStatus(
                runHealth.failed > 0 ? "warning" : "healthy",
              )}
            />
            <LiveStatCard
              label="Credits"
              ...
              status={asDashboardStatus(
                budget && budget.usage_percentage >= 80 ? "warning" : "healthy",
              )}
            />
          </LiveKpiGrid>
                ...
                stages={[
                  {
                    status: "pending",
                    count: runs.filter(
                      (r) => r.status === "created" || r.status === "queued",
                    ).length,
                  ...
                  },
                  {
                    status: "running",
                    count: runs.filter(
                      (r) => r.status === "running" && !r.completed_at,
                    ).length,
                    maxCount: runs.length,
                  },
                  {
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.85fr)]">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                    ...
                    <p className="mt-3 text-sm leading-6 text-slate-400">
                      One surface for deployments, execution, alerts, keys, and
                      delivery posture. No route shells pretending to be live
                      data.
                    </p>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <div className="grid gap-4">
                    <FlowStatusBar
                      stages={[
                        ...
                        {
                          status: "completed",
                          count: runs.filter((r) => r.status === "completed")
                            .length,
                          maxCount: runs.length,
                        },
                        {
                          status: "failed",
                          count: runs.filter((r) => r.status === "failed")
                            .length,
                          maxCount: runs.length,
                        },
                      ]}
            ...
            <div className="grid gap-4">
                  ...
                  <div className="grid gap-3">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-3 truncate text-xl font-semibold text-white">
                            {openclawBinding?.assistant_name ??
                              openclawRuntime?.label ??
                              "OpenClaw"}
                          </p>
                        ...
                        <StatusBadge
                          status={asDashboardStatus(openclawStatus)}
                          label={openclawStatus}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                        ...
                        <p className="mt-2 text-xs text-slate-500">
                          {openclawBinding?.model ??
                            "Model metadata syncs when the binding is available."}
                        </p>
                      </div>
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                        ...
                        <p className="mt-2 text-sm text-white">
                          {formatDateTime(
                            openclawRuntime?.last_synced_at ??
                              openclawRuntime?.last_seen_at,
                          )}
                        </p>
                        <p className="mt-2 text-xs text-slate-500">
              ...
              <LivePanel
                ...
                meta={`${activeWebhooks.length} active`}
              >
                <div className="grid gap-3">
                  <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                    <div className="flex items-center gap-2 text-slate-300">
                    ...
                    <p className="mt-2 text-sm text-slate-400">
                      {webhooks.length} configured webhook routes across the
                      operator scope.
                    </p>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                    <div className="grid gap-2 sm:grid-cols-2">
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <p className="mt-2 text-sm leading-6 text-slate-400">
                          Infra and model spend stay in separate surfaces so
                          operators can tell whether the cost problem is
                          deployment shape or token burn.
                        </p>
                      </div>
                    </div>
                  </div>
