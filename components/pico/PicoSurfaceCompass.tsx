...
function itemClasses(tone: PicoSurfaceCompassTone = 'secondary') {
  if (tone === 'primary') {
    return picoEmber('group grid gap-3 p-5 transition hover:border-(--pico-accent) hover:bg-[linear-gradient(180deg,rgba(117,64,33,0.28),rgba(45,27,16,0.32))]')
  }

  if (tone === 'soft') {
    return picoSoft('group grid gap-3 p-5 transition hover:border-(--pico-border-hover) hover:bg-[rgba(22,16,12,0.86)]')
  }

  return picoInset('group grid gap-3 p-5 transition hover:border-(--pico-border-hover) hover:bg-(--pico-bg-surface-hover)')
}
    ...
    <section className={picoPanel('p-5 sm:p-6')} data-testid="pico-surface-compass">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-(--pico-text-secondary) sm:text-base sm:leading-7">
            {body}
          </p>
        </div>
        ...
        {aside ? (
          <div className={picoSoft('p-4 sm:p-5')}>
            ...
            <p className="mt-3 text-sm leading-6 text-(--pico-text-secondary)">{aside}</p>
          </div>
        ) : null}
      </div>
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-(--pico-text-muted)">
                {item.note}
              </span>
              <span className="text-sm text-(--pico-accent)" aria-hidden="true">
                {t('forwardSymbol')}
              </span>
            </div>
            ...
            <p className="text-sm leading-6 text-(--pico-text-secondary)">{item.caption}</p>
          </Link>
        ))}
      </div>
    </section>
